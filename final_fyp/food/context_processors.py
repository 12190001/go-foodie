...
def notifications(request):
    customer = Notification.objects.filter(receiver = request.user)[::-1]
    allnotifications = Notification.objects.filter(receiver = 'manager')[::-1]
    return {'notification':allnotifications, 'customer_notify':customer}
